...
侧边栏可切换。**量化多因子为默认**，是经无偏回测验证的方式；另两种为可解释的规则法。

#### ① 量化多因子（默认，经回测验证）
四个因子（方向已统一为"越大越好"），**行业中性化**后横截面 z-score 等权合成，
**综合分 = 入围股内分位(0–100)**，越高越优：
...
| 价值（1/PB，**用收盘 PB**） | 越便宜越好 | +0.061 | baostock 收盘 |

**三层都做了行业均衡 + 盘中稳定**：
1. **选池**：全市场按 **成交额分行业分层**取 ~100 只代表股（不再按价值分预筛 → 不扎堆金融）；
2. **打分**：每个因子先**行业内去均值**（行业中性）再 z-score → 比的是"行业内相对优劣"；
3. **展示**：侧边栏「每行业最多 N 只」兜底分散。
4. 四因子**全部基于收盘数据**（价值因子用收盘 PB），**盘中重算名单不抖**。

> ✅ 因子在**无偏样本外回测**（含退市股、扣成本）中 IC 稳健、训练/检验同向。
> ⚠️ **诚实定性：这是「风格 edge」，不是「选股 alpha」。** 纯多头检验期可跑赢可买的宽基 ETF
> （沪深300/中证500/1000）约 +3~5%/年，但**对同口径等权全市场无超额**——超额主要来自
> 「等权 + 中小盘 + 防御」风格。**实用前提**：你本来就买宽基 ETF；**风险**：大盘/高 beta 领涨时会失效。
> 因子最强的部分在空头（散户做不了）。详见 `strategy.py` / CHANGELOG。
...
| **长线价值** | **0.57** | 0.31 | 0.12 | 长期持有，基本面主导 |

> 💡 短线权重据回测校准（估值便宜最有效、反转技术为辅；追涨式动量反而与未来收益负相关）。
> 此两种为透明可解释的规则打分，未做纯多头落地验证——严肃使用请优先看①的诚实定性。
...
with st.sidebar:
    ...
    profile_label = st.radio(
        ...
        help="量化多因子：低波动+反彩票(避妖股)+中期反转+价值，横截面等权合成——"
             "这几个因子经无偏样本外回测验证有选股力（综合分=入围股内分位）；"
             "短线波段：估值分位主导+反转技术；长线价值：基本面主导。")
...
if ss.loaded:
    ...
    with hc1:
        if profile == "量化多因子":
            st.caption(f"风格：{profile_label}　|　综合分=入围 {len(ss.enrich)} 只内的"
                       f"**多因子横截面分位**(0-100，越高越优)。因子需 baostock 历史，"
                       f"故在「价值分 top{VAL_TOP}」预筛池内排序。")
        else:
    ...
    if profile == "量化多因子":
        # 量化模式：展示构成因子（综合分=横截面分位）
        core_cols = ["代码", "名称", "行业", "最新价", "PB",
                     "波动率", "最大日涨幅", "动量60", "综合分", "建议"]
    else:
